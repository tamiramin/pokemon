...
export default async function Page({ params }: { params: { slug: number } }) {
    ...
    return (
        <Container bg={color[0]} width={'100vw'} height={'100vh'} pt={'3em'} pl={0} pr={0}>
            <Flex justifyContent={'space-between'} p={'1.5em'}>
                <Flex flexDirection={'column'}>
                    <Text fontSize={'5xl'} as={'b'} pt={'0.5em'} pb={'0.25em'} >{pokemon.name}</Text>
                    <Flex pl={'0.5em'} gap={2}>
                        {pokemon.types.map((tipe:any) => (
                        <Box bgColor={color[1]} borderRadius={'3xl'} p={'0.8em'} pt={'0.2em'} pb={'0.2em'}>
                            <Text fontSize={'lg'}>{tipe.type.name}</Text>
                        </Box>
                        ))}
                    </Flex>
                </Flex>
            ...
            <Flex justifyContent={'center'} alignItems={'center'} mt={'1.5em'} zIndex={99}>
            ...
            <Flex width={'100%'} height={'50vh'} bg={'#f2f3f2'} mt={'-3em'} borderTopRadius={'3xl'} p={'1em'} >
